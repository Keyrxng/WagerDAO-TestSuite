...
contract betManager_V01 {
    ...
    function setNftFeesDistributor(
    ...
    function createBet(
        ...
        require(
            ...
            "Only 1 2 or 3 accepted for footbal."
        ); // 1 = Team1 win, 2 = Team2 win, 3 = Equal result between teams.
        ...
        require(
            currentTime <= lastMinutesBuffer,
            "Bet time for this match expired."
        );

        // Depositor must approve tokens transfer on behalf of this contract first
        require(
            scoreContract.transferFrom(msg.sender, address(this), betAmount),
            "$SCORE tokens transfer failed."
        );
    ...
    function claimWinning(uint256 betID) public {
        ...
        require(
            msg.sender == betDetails.originalBettor,
            "This betID belongs to someone else."
        ); // Check if msg sender is the original bettor for current ID.
        require(
            dailyDetails.result > 0,
            "Result not declared by administrator yet."
        ); // Check if result is declared by administrator.
        ...
        uint256 userPoolSharePercent = ((betDetails.amountOfBet * multiplier) /
            pooledForThisOutcome) * 100;
        calculateUserPoolWeight =
            (totalPooledAmountForThisMatch *
                (userPoolSharePercent / multiplier)) /
            100;

        remainderPercent = userPoolSharePercent % multiplier;
        remainderPercentAdjuted = (remainderPercent * 100) / multiplier;
        remainderAmount =
            (totalPooledAmountForThisMatch * remainderPercentAdjuted) /
            divider;
        userWeightAdjusted = calculateUserPoolWeight + remainderAmount;

        require(
            betDetails.outcomePrediction == dailyDetails.result,
            "Your prediction was not accurate."
        ); // Check user prediction vs actual result.
        ...
        require(
            ...
            "More than the total funds available in this bet pool."
        ); // Prevent extra spending.
        ...
        if (fees > 0) {
            try scoreContract.transfer(teamWallet, teamShare) {} catch Error(
                string memory reason
            ) // Transfer withdraw fees to team address.
            {
            ...
            try
                scoreContract.transfer(devWallet, developmentShare)
            {} catch Error(
                string memory reason
            ) // Transfer withdraw fees to development address.
            {
            ...
            try
                scoreContract.transfer(treasuryWallet, treasuryShare)
            {} catch Error(
                string memory reason
            ) // Transfer withdraw fees to treasury address.
            {
        ...
        require(
            scoreContract.transfer(msg.sender, amountToUser),
    ...
    function getBetDetails(
        ...
        returns (
            ...
            uint256 BetOutcomePrediction,
            uint256 BetTime,
            uint256 betMatch,
            uint256 winAmount,
            uint256 betNumb,
            bool isAdminIntervened,
            bool isItWin,
            bool IsItClaimed
        )
    {
        ...
        )
    {
        ...
        (pooledTm1, pooledTm2, pooledEql, pooledTotal) = getPooledAmounts(
            matchNumber
        );
        return (
            b.Team1,
    ...
    function rescueStuckFees() public onlyAdministrator {
        ...
        contractFees = 0;
        require(
            scoreContract.transfer(msg.sender, tokensToRescue),
            "Transfer failed"
        );
    }
    ...
    function manageWalletsAndProportions(
        address newTeamW,
        address newTreasuryW,
        address devW,
        uint256 percentTreasury,
        uint256 percentDev,
        uint256 percentTeam
    ) public onlyAdministrator {
        require(
            percentTreasury + percentDev + percentTeam == 100,
            "Sum of proportions should be always 100."
        );
    ...
    function createMatch(
        string memory newTeam1,
        string memory newTeam2,
        uint256 startTime,
        uint256 endTime
    ) public onlyAdministrator {
        require(
            startTime >= block.timestamp + 3 minutes,
            "Betting should start at least 3 minutes before match."
        );

        require(
            endTime > startTime,
            "End match time must be greater than start match time."
        ); // Prevent errors with times inserted.
        // 105 minutes in production
        require(
            endTime - startTime >= 105 minutes,
            "Each match should be atleast 105 minutes."
        ); // Prevent errors with times inserted.
        // 180 minutes in production
        require(
            endTime - startTime <= 180 minutes,
            "Duration should be less than 180 miutes."
        ); // Prevent errors with times inserted.
    ...
    function declareMatchOutcome(
        ...
        require(
            !dailyBetsID[matchID].adminDeclaredResult,
            "Admin already set result for this match."
        );
        require(
            matchID > 0 && matchID <= totalBetsCreated,
            "ID out of bounds."
        );
        require(
            timeBuffer >= dailyBetsID[matchID].matchEnd,
            "Cannot declare result before match end."
        );
        require(
            matchResult == 1 || matchResult == 2 || matchResult == 3,
            "Declared result can be either 1, 2 or 3."
        );
        ...
        if (matchResult == 1 && dailyBetsID[matchID].pooledTeam1 == 0) {
            dailyBetsID[matchID].totalClaimed = dailyBetsID[matchID]
                .totalPooledBets;
            try
                scoreContract.transfer(
                    treasuryWallet,
                    dailyBetsID[matchID].totalPooledBets
                )
            {} catch Error(
                string memory reason
            ) // Transfer to treasury address if noone put bet for this outcome.
            {
                emit ErrorReason(reason);
            }
    ...
    function changeFees(
        ...
        uint256 _newNftRewardFee
    ) external onlyAdministrator {
        require(
            _newWithdrawFee <= 100 && _newNftRewardFee <= 100,
            "Fees cannot be set above 10% each."
        );
    ...
    function refundUserBet(uint256 ID) external onlyAdministrator {
        ...
        if (contractBalances >= currentBetAmount) {
            ...
            require(
                ...
                "Transfer from main token failed."
            );
    ...
    function refundMultipleBets(
        ...
        require(
            fromID < toID,
            "Start number must be lower than finish number."
        );
