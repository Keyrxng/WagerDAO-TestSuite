...
//         cWagerPass = new Wager_DAO_NFT(address(cTreasury));
//         cTreasury.setScoreContract(address(cScores));
//         cTreasury = new WagerDAOTreasury(address(cScores), address(cWagerPass));

//         cFeeDistro = new NFTBetFeesDistributor(address(cScores), address(cWagerPass), address(cBetManager)); // requires betManager > not goot
//         cBetManager = new betManager_V01(address(cScores), address(cFeeDistro)); // requires feeDistro > not good
        
//         cBetManager.setNftFeesDistributor(address(cFeeDistro)); // manually set feeDistro as constructor must be a false input
